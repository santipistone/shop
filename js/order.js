...
function loadOrder(a, b, c, d) {
    ...
    hot1.href = "home.php?page=item&item="+d;
...
function getValue(resp) {
    prezzo = prezzo + resp[0].price*1;
    loadOrder(resp[0].img, resp[0].nome, resp[0].price, resp[0].codice);
...
function loadOrderPrice() {
    ...
    for (let i=0; i<shop2.length; i++) {
        fetch("script/price.php?item="+shop2[i]).then(onRes).then(getValue);
